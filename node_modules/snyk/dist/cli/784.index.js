...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.resolveAndTestFactsRegistry = exports.resolveAndTestFactsUnmanagedDeps = exports.resolveAndTestFacts = void 0;
const spinner_1 = __webpack_require__(86766);
...
async function resolveAndTestFacts(ecosystem, scans, options) {
    ...
    const featureFlagEnabled = await feature_flags_1.hasFeatureFlag('snykNewUnmanagedTest', options);
...
async function fetchIssues(start_time, dep_graph_data, component_details, orgId) {
    var _a, _b;
    const response = await polling_test_1.getIssues({
    ...
    const issues = response.data.result.issues.map((issue) => {
        ...
        converted.fixInfo = utils_1.convertToCamelCase(converted.fixInfo);
        return converted;
    });
    const issuesData = utils_1.convertMapCasing(response.data.result.issues_data);
    const depGraphData = utils_1.convertDepGraph(response.data.result.dep_graph);
    ...
    const fileSignaturesDetails = utils_1.convertMapCasing(response.data.result.file_signatures_details);
    return {
        issues,
        issuesData,
        depGraphData,
...
async function resolveAndTestFactsUnmanagedDeps(scans, options) {
    ...
    if (orgId === '') {
        const self = await utils_1.getSelf();
    ...
    for (const [path, scanResults] of Object.entries(scans)) {
        ...
        for (const scanResult of scanResults) {
            try {
                ...
                const { issues, issuesData, depGraphData, dependencyCount, depsFilePaths, fileSignaturesDetails, } = await fetchIssues(start_time, dep_graph_data, component_details, orgId);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.makeSelfRequest = exports.getSelf = exports.convertDepGraph = exports.convertObjectArrayCasing = exports.convertMapCasing = exports.convertToCamelCase = void 0;
...
function getSelf() {
    return makeSelfRequest().then((res) => {
        var _a;
        const response = JSON.parse(res.body);
        return (_a = response === null || response === void 0 ? void 0 : response.data) === null || _a === void 0 ? void 0 : _a.attributes;
    });
}
...
exports.makeSelfRequest = makeSelfRequest;
...
async function getIssues(issuesRequestAttributes, orgId) {
    const payload = {
        method: 'POST',
        url: `${config_1.default.API_REST_URL}/orgs/${orgId}/unmanaged_ecosystem/issues?version=2022-06-29~experimental`,
        json: true,
        headers: {
    ...
    const result = await promise_1.makeRequest(payload);
...
async function getDepGraph(id, orgId) {
    const payload = {
        method: 'GET',
        url: `${config_1.default.API_REST_URL}/orgs/${orgId}/unmanaged_ecosystem/depgraphs/${id}?version=2022-05-23~experimental`,
        json: true,
        headers: {
            'Content-Type': 'application/vnd.api+json',
            'x-is-ci': is_ci_1.isCI(),
...
async function createDepGraph(hashes, orgId) {
    const payload = {
        method: 'POST',
        url: `${config_1.default.API_REST_URL}/orgs/${orgId}/unmanaged_ecosystem/depgraphs?version=2022-05-23~experimental`,
        json: true,
        headers: {
            'Content-Type': 'application/vnd.api+json',
            'x-is-ci': is_ci_1.isCI(),
...
async function snykHttpClient(requestInfo) {
    ...
    return index_1.makeRequest(payload);
}
...
function prepareEcosystemResponseForParsing(payload, response, options) {
    ...
    return {
        ...
        hasUnknownVersions: dep_graph_1.hasUnknownVersions(depGraph),
    };
}
...
async function sendAndParseResults(payloads, spinnerLbl, root, options) {
    ...
    for (const payload of payloads) {
        ...
        const { depGraph, payloadPolicy, pkgManager, targetFile, projectName, foundProjectCount, displayTargetFile, dockerfilePackages, platform, scanResult, hasUnknownVersions, } = prepareResponseForParsing(payloadCopy, res, options);
        const ecosystem = ecosystems_1.getEcosystem(options);
